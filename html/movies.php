...
$movies = $pdo->query("SELECT * FROM media_cache WHERE type='movie' ORDER BY title ASC")->fetchAll(PDO::FETCH_ASSOC);
$cacheEmpty = empty($movies);

// Última actualización del caché
$lastUpdate = $pdo->query("
    SELECT MAX(updated_at) FROM media_cache WHERE type='movie'
")->fetchColumn();
?>
